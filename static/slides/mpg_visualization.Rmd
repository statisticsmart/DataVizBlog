...
Check the video lectures in the Google Classroom.
...
This question is based on `mpg` dataset in `ggplot2` package. Load the `ggplot2` package and `mpg` dataset using the following command.
...
To get summary statistics use the command
...
## Write R codes to reporduce the following plots using both `qplot` function and `ggplot` function in the `ggplot2` package. The first one is done for you.
